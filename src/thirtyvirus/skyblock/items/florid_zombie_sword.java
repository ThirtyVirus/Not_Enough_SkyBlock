...
public class florid_zombie_sword extends UberItem {

    public florid_zombie_sword(Material material, String name, UberRarity rarity, boolean stackable, boolean oneTimeUse, boolean hasActiveEffect, List<UberAbility> abilities, UberCraftingRecipe craftingRecipe, String raritySuffix) {
    ...
    public boolean rightClickAirAction(Player player, ItemStack item) {
        ...
        for (Player other : Bukkit.getOnlinePlayers()) {
            if (other.getWorld().equals(player.getWorld())) {
                if (player.getLocation().distance(other.getLocation()) <= 8 && !other.equals(player)) {
                    Utilities.safeHeal(other, 3);
                }
            }
        }
